...
export class FloorService {
  ...
  public static async getAllFloors(buildingId?: string, page = 1, pageSize = 100): Promise<FloorsResponse[]> {
    try {
      const res = await axiosClient.get<{ items: FloorsResponse[] }>(`${API_URL}`, {
  ...
  public static async getFloorById(id: string): Promise<FloorDetailsResponse> {
    try {
      const res = await axiosClient.get<FloorDetailsResponse>(`${API_URL}/${id}`);
      return res.data;
    } catch (error) {
  ...
  public static async createFloor(data: CreateFloorRequest): Promise<CreateFloorResponse> {
    try {
      const res = await axiosClient.post<CreateFloorResponse>(`${API_URL}`, data);
      return res.data;
    } catch (error) {
  ...
  public static async updateFloor(id: string, data: UpdateFloorRequest): Promise<UpdateFloorResponse> {
    try {
      const res = await axiosClient.put<UpdateFloorResponse>(`${API_URL}/${id}`, data);
      return res.data;
    } catch (error) {
  ...
  public static async deleteFloor(id: string): Promise<DeletedFloorResponse> {
    try {
      const res = await axiosClient.delete<DeletedFloorResponse>(`${API_URL}/${id}`);
      return res.data;
    } catch (error) {
