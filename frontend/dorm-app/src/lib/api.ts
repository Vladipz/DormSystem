...
async function handleResponse<T>(response: Response): Promise<T> {
  ...
  // Handle successful responses with no content (204, or 200 with empty body)
  if (
    response.status === 204 ||
  ...
  if (response.ok && !hasJsonContent) {
    ...
    // If there's text but it's not JSON, return it as is
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      // For successful responses that can't be parsed as JSON
      if (response.ok) {
        return undefined as T;
      }
      // This catches JSON parse errors for error responses
